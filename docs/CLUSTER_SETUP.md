...
> **Quick reference:** see [`docs/SETUP_COMMANDS.md`](SETUP_COMMANDS.md) for
> copy-paste master/slave commands only.
>
...
> If `~/kmeans-parallel` is not the repo path on a node, pass
...
| `none of the listed hosts is THIS machine` / stale `node0` | DHCP changed VM IPs; `/etc/hosts` still has old addresses. On the master: `ROLE=master NODE_IPS="<this-ip> <w1> <w2>" scripts/bootstrap_node.sh` then re-run `run_demo.sh`. |
